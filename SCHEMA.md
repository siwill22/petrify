...
{ "label": "View the code",
  ...
  "figures": [{ "url": "provenance/<figure>.png",
                "caption": "What this shows, and -- if it did not come from the "
                           "steps above -- where it actually came from." }],
...
`figures` bundles static images as supporting evidence for `steps` — via
`view.notebook(path, figures=[{"path": ..., "caption": ...}, ...])`. Each file is
copied into the export as-is; nothing checks a figure's content against `steps` or
against the view, so **the caption is the only thing standing between this and an
implied audit trail it does not have**. A figure generated by a genuinely separate
analysis — a different dataset, a different script, in a different repository —
must say so in its own caption, plainly, the same way `cites()` names a package it
cannot show directly rather than staying silent about it.
